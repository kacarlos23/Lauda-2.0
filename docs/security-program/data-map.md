...
| Autenticacao e sessao | E-mail, senha, access/refresh JWT, role, tenant, permissoes, sid/jti, hash HMAC, IP e User-Agent | Usuario | Credencial/seguranca | Login, refresh, `/auth/me`, logout | Tokens no cliente; `AuthSession`, `RefreshTokenFamily` e hashes `RefreshToken` no PostgreSQL | Usuario; backend | SecureStore nativo; `localStorage` web | Dispositivo do usuario; backend TBD | Access 15m; refresh/sessao 7d renovavel; registros expirados + 30 dias propostos, purge pendente | Seguranca TBD | Engenharia TBD | ADR de sessoes; rotacao/reuse atomicos; logout servidor; `tokenService.test.ts`/`auth.test.ts`. |
...
| Credenciais e tokens | Senha hash, JWT, HMAC de refresh, PIN/HMAC reset, peppers/secrets | Seguranca/credencial | Sessao/rotacao implementadas; aprovar purge, alertas e risco de localStorage web. |
