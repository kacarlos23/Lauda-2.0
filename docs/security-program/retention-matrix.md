...
| Refresh token | JWT no cliente | 7 dias por default, sem revogacao servidor | TBD; preferir rotacao/revogado em servidor | Etapa 2 deve criar sessao persistente e revogacao | Seguranca/Produto TBD | Insuficiente |
...
| AdminAuditLog | Ator, role, acao, recurso, payload, tenant, timestamp | Sem prazo aprovado | TBD por seguranca/auditoria | Redaction adicional; retencao com legal hold quando aplicavel | Seguranca/Juridico TBD | Pendente |
| Logs tecnicos | Erros, stdout/stderr, possiveis payloads | Arquivos locais no repo/dev; provider prod TBD | TBD por seguranca/operacao | Redaction, request IDs, expiracao automatica | Engenharia/Infra TBD | Pendente |
...
| Backups | Todos os dados persistidos | Nao configurado no repo | TBD com RPO/RTO | Expiracao, criptografia, restore e reaplicacao de delecoes | Infra/Data Owners TBD | Bloqueante |
