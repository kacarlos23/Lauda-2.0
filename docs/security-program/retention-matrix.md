...
| Notificacoes persistentes | Tipo, destinatario, ator, recurso, payload minimizado e leitura | Expiracao tecnica em 90 dias e limpeza pelo dispatcher | 90 dias, sujeito a aprovacao de Data Owner/Juridico | Exclusao automatica por `expiresAt`; eventos outbox publicados permanecem sujeitos a prazo operacional TBD | Produto/Operacao/Juridico TBD | Parcial; inbox implementado, prazo juridico pendente |
| Dispositivos e entregas push | Token Expo, plataforma, versao, tickets e recibos | Ate logout, invalidacao ou substituicao do token; historico de entrega sem prazo separado | Curto prazo para diagnostico, a aprovar | Logout/rotacao/`DeviceNotRegistered` desativam token; purge de entregas pendente | Engenharia/Privacidade TBD | Parcial |
