...
| Expo/React Native ecosystem | App Expo com SecureStore, FileSystem, ImagePicker, Sharing; cloud Expo/EAS nao confirmado | Tokens no dispositivo, avatar escolhido, PDFs em cache local; dados de build se EAS usado | Dispositivo do usuario; cloud TBD se habilitado | Cache/dispositivo sob controle local; cloud TBD | Fornecedor de app/runtime TBD | Confirmar se EAS Build/Updates/Notifications/Analytics serao usados; se sim, abrir vendor review. |
