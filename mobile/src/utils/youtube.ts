...
export function buildYouTubeEmbedUrl(videoId: string): string {
  if (!YOUTUBE_VIDEO_ID_PATTERN.test(videoId)) throw new Error("ID de vídeo do YouTube inválido");
  return `https://www.youtube.com/embed/${videoId}?autoplay=1&playsinline=1`;
}

export function isAllowedYouTubePlayerNavigation(value: string, videoId: string): boolean {
  if (value === "about:blank") return true;
  if (!YOUTUBE_VIDEO_ID_PATTERN.test(videoId)) return false;
