...
export default function EditSongScreen() {
  ...
  return <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }} edges={["left", "right"]}><SongForm initial={viewState.song} backHref={`/songs/${id}`} saving={saving} error={error} onSave={async (payload) => (await updateSong(id, payload)).id} /></SafeAreaView>;
}
